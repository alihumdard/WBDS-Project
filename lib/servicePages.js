...
export function extractFaqEntries(contentHtml = "") {
  ...
  while ((match = blockRegex.exec(contentHtml)) !== null) {
    blocks.push({ tag: match[1].toLowerCase(), innerHtml: match[2], text: stripTags(match[2]) });
  }

  const faqHeadingIndex = blocks.findIndex(
    (block) => block.tag.startsWith("h") && /^(frequently asked questions|faqs?)$/i.test(block.text)
  );
  if (faqHeadingIndex === -1) return [];

  const faqBlocks = blocks.slice(faqHeadingIndex + 1);

  const isQuestionBlock = (block) => {
    if (block.tag.startsWith("h")) return true;
    const trimmedInner = block.innerHtml.trim();
    const strongMatch = trimmedInner.match(/^<strong[^>]*>([\s\S]*?)<\/strong>/i);
    return Boolean(strongMatch) && stripTags(strongMatch[1]).length > 0;
  };
  ...
  for (const block of faqBlocks) {
    // A following top-level heading (e.g. "Conclusion") marks the end of the FAQ list.
    if (block.tag.startsWith("h") && /conclusion/i.test(block.text)) {
      break;
    }

    if (isQuestionBlock(block)) {
      pushCurrent();
      currentQuestion = block.text.replace(/\?*$/, "?");
      currentAnswerParts = [];
